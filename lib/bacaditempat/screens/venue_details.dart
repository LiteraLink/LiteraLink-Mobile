...
class _VenueDetailState extends State<VenueDetail> {
  ...
  Future<List<BookVenue>> fetchNotAvailableBookVenue() async {
    var url =
        Uri.parse('$baseUrl/get-product-notavailable/${widget.venue.pk}/');
    var response =
        await http.get(url, headers: {"Content-Type": "application/json"});
    ...
    if (response.statusCode == 200) {
      ...
      for (var d in data) {
        if (d != null) {
          ...
          if (book.fields.username == loggedInUser.username) {
  ...
  fb.Book convertBookVenueToBook(BookVenue bookVenue) {
    return fb.Book(
      pk: bookVenue.pk,
      fields: fb.Fields(
        bookId: bookVenue.fields.bookId,
        title: bookVenue.fields.title,
        authors: bookVenue.fields.authors,
        displayAuthors: bookVenue.fields.displayAuthors,
        description: bookVenue.fields.description,
        categories: bookVenue.fields.categories,
        thumbnail: bookVenue.fields.thumbnail,
      ),
    );
  }
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: ClipRRect(
                        ...
                        child: Image.network("https://i.imgur.com/03jbH1r.jpg"),
                      ),
                    ),
                                ...
                                child: FutureBuilder(
                                  ...
                                  builder: (context, snapshot) {
                                    if (snapshot.connectionState ==
                                      ...
                                      final List<BookVenue> userBooks =
                                          snapshot.data!.toList();

                                      if (userBooks.isEmpty) {
                                        return const Text(
                                            "Anda tidak memiliki buku yang dibooking.");
                                      }
