# LiteraLink ![](https://i.imgur.com/2h5tvk6.png)
...
> [Georgina Elena Shinta Dewi Achti](https://github.com/georginaelena) `(2206810995)` <br>
> [Muhammad Pendar Bintang Kasdiono](https://github.com/Pebintk) `(2206083174)` <br>
...
[LiteraLink](https://install.appcenter.ms/orgs/pbp-deploy/apps/literalink/distribution_groups/public)
...
3.  <b>BacaDiTempat ([Elena](https://github.com/georginaelena))</b><br/>
    ...
    - `CREATE` Membuat Venue baru untuk pilihan lokasi Coffee Shop venue yang bekerja sama dengan LiteraLink
    - `CREATE` Melakukan Booking sesuai keinginan user
    - `DELETE` Menghapus Venue sesuai kebutuhan admin
    - `DELETE` Mengembalikan buku setelah user selesai menggunakan buku sehabis booking
    - `UPDATE` Mengedit Venue sesuai kebutuhan admin
    - `READ` Menampilkan buku dalam katalog sesuai dengan *sort*, *filter*, dan *display*
    - `READ` Menampilkan halaman buku tersendiri
4.  <b>DimanaSajaKapanSaja ([Martin](https://github.com/MartinTarigan))</b><br/>
    ...
    - `CREATE` Menambahkan Station (Admin)
    - `CREATE` Meminjam buku yang tersedia di station (Member)
    - `UPDATE` Mengedit atribut station (Admin)
    - `DELETE` Menghapus station (Admin)
    - `READ` Menampilkan station-station yang terdaftar beserta buku-buku dalam station(Admin)
    - `READ` Menampilkan Buku-buku yang dipinjam (Member)
5.  <b>Bibliofilia ([Pendar](https://github.com/Pebintk))</b><br/>
    ...
    - `CREATE` Membuat *post* baru di forum
    - `CREATE` Memberikan balasaan untuk suatu forum
    - `UPDATE` Memperbarui jumlah balasan yang dimiliki suatu forum
    - `DELETE` Menghapus *post* atau balasan yang dilakukan oleh admin
    - `READ` Menampilkan halaman utama forum
    - `READ` Menampilkan halaman balasan suatu forum
...
## Panduan _Build_ ⚙️🛠
### Instalasi Flutter untuk membangun dan menjalankan proyek:
...
- Buka _console window_ atau terminal yang memiliki direktori Flutter di _path_-nya, jalankan perintah `flutter doctor` untuk melihat apakah ada dependensi platform yang diperlukan untuk menyelesaikan _setup_.

- Penggunaan IDE Android Studio direkomendasikan untuk mengembangkan proyek ini. Namun IDE lain seperti Visual Studio Code juga dapat digunakan.

### Menjalankan program secara lokal
- Inisialisasi awal untuk menjalankan program adalah melakukan _clone repository_ dengan menjalankan `git clone` pada direktori yang diinginkan.

- Untuk menjalankan proyek secara lokal, jalankan perintah `flutter run` pada direktori proyek dan pilih tempat untuk menjalankan aplikasi (misalnya browser Windows, Chrome, dll.). Jika dijalankan di browser, aplikasi dapat diakses melalui localhost.
